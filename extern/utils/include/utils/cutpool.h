...
auto cutptr_equal = [](const CutPtr& x, const CutPtr& y)
{
...
auto cutptr_hash = [](const CutPtr& x)
{
	return x->sig;
};
...
{
protected:
	using Pool = std::unordered_set<CutPtr, decltype(cutptr_hash), decltype(cutptr_equal)>;
public:
	CutPool();
